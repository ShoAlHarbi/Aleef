...
async function edit (){
  ...
  await firebase.database().ref('/MissingPetPosts/'+postid).update({
    AnimalType: AniType,
    PetPicture: AniPic,
    latitude: AniLat,
    longitude: AniLong,
  }).then(async(data) => {
    await new profile()._onRefresh();
    Alert.alert('', 'تم حفظ التغييرات بنجاح، يرجى تحديث الصفحة',[{ text: 'حسناً'}])
  });
  await new profile()._onRefresh();
  await new profile().render();
  nearUsers();   
